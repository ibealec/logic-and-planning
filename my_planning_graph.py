...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        # Using a set here since we wont want any duplicates
        s_nodes = set()
        previous_lev = level-1
        for node_a in self.a_levels[previous_lev]:
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Test if action in node 1 gets cancelled by node 2's action
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Test if action of node_a1 negates precondition of node_a2
        for effect_1 in node_a1.action.effect_add:
            for precond_2 in node_a2.action.precond_neg:
                if effect_1 == precond_2:
                    return True
        # Test if action of node_a2 negates precondition of node_a1
        for effect_2 in node_a2.action.effect_add:
            for precond_1 in node_a1.action.precond_neg:
                if precond_1 == effect_2:
                    return True

        # Now the same thing for removing effects with positive preconditions
        for effect_1 in node_a1.action.effect_rem:
            for precond_2 in node_a2.action.precond_pos:
                if effect_1 == precond_2:
                    return True
        for effect_2 in node_a2.action.effect_rem:
            for precond_1 in node_a1.action.precond_pos:
                if precond_1 == effect_2:
                    return True
    ...
    def update_s_mutex(self, nodeset: set):
        ...
        """
        nodelist = list(nodeset)
        for i, node_1 in enumerate(nodelist[:-1]):
            for node_2 in nodelist[i + 1:]:
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for precond_s1 in node_s1.parents:
            for precond_s2 in node_s2.parents:
                if precond_s1.is_mutex(precond_s2) is False:
                    return False
    ...
    def h_levelsum(self) -> int:
        ...
        :return: int
        """
        def state_to_literal(s_node: PgNode_s):
            """ Helper function to turn a state in s_levels into a literal expression
            """
            if s_node.is_pos:
        ...
        for goal in self.problem.goal:
            ...
            for level in range(len(self.s_levels)):
                for state in self.s_levels[level]:
                    literal = state_to_literal(state)
                    if goal == literal:
                        goal_found = True
                        level_sum += level
                        break
